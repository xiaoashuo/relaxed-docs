...
Relaxed 提供了 `DesensitizationHandlerHolder` 类，来对系统内的所有脱敏处理器进行归集，方便使用的时候直接获取，而不必再创建新的对象。

### 3.2 简单脱敏

对于简单脱敏类型，内置了一种脱敏处理器：`SixAsteriskDesensitizationHandler`，不管原文是什么，一律返回6个 *。
...
默认提供了 Email 类型的脱敏正则，所以可以使用以下代码来简化正则的定义：
...
和正则脱敏一样， 默认提供了 PhoneNumber 类型的滑动规则，所以可以使用以下代码来简化：
...
在 web 服务中，服务端的响应数据很多情况下都是 json 数据，为了更方便的进行数据脱敏， 提供了以下三种脱敏注解。
...
如果需要使用 json 注解脱敏，则需要将`JsonSerializerModifier` 注册到 Jackson 的 `ObjectMapper` 中。
...
b) 利用 java 的 SPI 机制来加载简单脱敏处理器

所以只需在项目的 `resources/META-INF/services` 目录下新建名为

`com.relaxed.common.desensitize.handler.SimpleDesensitizationHandler` 的文件。
